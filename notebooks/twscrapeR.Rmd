...
4.  **Get replies**: descarga las respuestas de los tweets con la función tweet_replies() .
4.  **Get replies avanzadas **: descarga las respuestas de los tweets de un dataset desde los n días especificados tras la publicación de cada tweet, con una frecuencia especificada. Usa la función search_tweets (). Adicionalmente, descarga los comentarios de los comentarios y obtiene hilos de conversaciones.
...
#### Cargar librerías y comprobar versión de twscrape
...
tryCatch({
  ...
  if (version_a_numeric(version_actual) < version_a_numeric("0.19.0")) {
    message("twscrape ", version_actual, " es inferior a 0.19.0. Actualizando...")
    system2(python_exe, args = c("-m", "pip", "install", "twscrape>=0.19.0"))
  } else {
    message("twscrape ", version_actual, " ya cumple el requisito (>=0.19.0).")
  }
  
}, error = function(e) {
  message("twscrape no encontrado. Instalando...")
  system2(python_exe, args = c("-m", "pip", "install", "numpy", "twscrape>=0.19.0"))
})
setup_twscraper()
...
frecuency <- "n intervalo"      # Intervalo  <- min | hour | day | week | month| year
sleep_time <- 5                 # Tiempo entre peticiones en segundos
...
if (length(date_sequence) > 1){
  for (i in 1:(length(date_sequence) - 1))  {
    ...
    # Fecha para la query
    ini_date = paste0(gsub(" ", "_", date_sequence[[i]]))
    end_date = paste0(gsub(" ", "_", date_sequence[[i+1]]))
    # Query
    ...
    tweets <- search_tweets(query_date, n = 900, product = "Top")
    if (length(tweets) >= 1){
...
print(glue("Duración total: {round(elapsed, 2)} minutos"))
```
...
for (i_user in 1:(length(list_users))){
  ...
  if (length(date_sequence) > 1){
    for (i_date in 1:(length(date_sequence) - 1))  {
      ...
      # Fecha para la query
      ini_date = paste0(gsub(" ", "_", date_sequence[[i_date]]))
      end_date = paste0(gsub(" ", "_", date_sequence[[i_date+1]]))
      # Query
      print(glue("Desde {ini_date} hasta {end_date}"))
...
```{r Get-Retweets, message=TRUE, warning=TRUE}
# Parámetros
data_path = "../data"
dataset <- "xxxxxxxx"  # Nombre del dataset (directorio)
prefix <- "xxxxxxxx"   # Prefijo del fichero para los datos
min_RTs <- 1           # Mínimo número de RTs para descargar RTs
sleep_time <- 5        # Tiempo entre peticiones en segundos
...
# Seleccionar tweets para descargar sus RTs
...
Usa la función tweet_replies(). Por este método se obtendrán como máximo 300 respuestas de un mensaje

```{r Get-replies, message=TRUE, warning=TRUE}

# Parámetros
data_path = "../data"
dataset <- "xxxxxxxx"           # Nombre del dataset (directorio)
prefix <- "xxxxxxxx"            # Prefijo del fichero para los datos
min_replies <- 1                # Mínimo número de comentarios para descargar
sleep_time <- 5                 # Tiempo entre peticiones en segundos
...
for (i in 1:(length(list_tweets_id))){
  if (list_tweets_id[[i]] > last_tweet_id){
    ...
    tweets <- tweet_replies(id_actual, n = 10000)
    if (length(tweets) >= 1){
      df_tweets <- to_dataframe(tweets) %>%
...
#### Get replies avanzadas

Usa la función search_tweets() utilizando frecuencia de descarga. Se recomienda cuando se tienen que descargar mensajes que tienen más de 300 respuestas. 
Adicionalmente, descarga los comentarios de los comentarios y obtiene hilos de conversaciones.

```{r Get-replies_2, message=TRUE, warning=TRUE}
# Parámetros
data_path = "../data"
dataset <- "xxxxxxxx"           # Nombre del dataset (directorio)
prefix <- "xxxxxxxx"            # Prefijo del fichero para los datos
...
for (i in 1:(length(list_tweets_id))){
  if (list_tweets_id[[i]] > last_tweet_id){
    ...
    if (length(date_sequence) > 1){
      for (i_date in 1:(length(date_sequence) - 1))  {
        # Fecha para comprobar rango 
        min_date <- date_sequence[[i_date]]
        max_date <- date_sequence[[i_date+1]]
        # Fecha para la query
        ini_date <- format(date_sequence[[i_date]],   "%Y-%m-%d_%H:%M:%S_UTC", tz = "UTC")
        end_date <- format(date_sequence[[i_date+1]], "%Y-%m-%d_%H:%M:%S_UTC", tz = "UTC")
        query <- glue("conversation_id:{id_actual} filter:replies since:{ini_date} until:{end_date}")
        print(glue("--> Descargando {query} ......"))
        tweets <- search_tweets(query, n = 900, product = "Top")
        if (length(tweets) >= 1){
          df_tweets <- to_dataframe(tweets) %>%
          mutate(url_replied = list_tweets_url[[i]]) %>%
          select(id,date,username,text,user_displayname,user_id,reply_count,retweet_count,like_count,quote_count,views_count,lang,url,user_followers,user_verified, url_replied)
