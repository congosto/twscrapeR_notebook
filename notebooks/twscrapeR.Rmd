...
```{r parche twscrape}
# 1. Especificar entorno ANTES de cargar la librería
use_miniconda <- FALSE
# Opción A: miniconda
# Se asume que se ha instalado un entorno llamado "reticulate" en miniconda
if (use_miniconda) {
...
# 2. Instalar y cargar reticulate
if (!"reticulate" %in% installed.packages()) { install.packages("reticulate") }
library(reticulate)

# 3. Especificar entorno conda (solo una vez, después de library)
if (use_miniconda) {
  reticulate::use_condaenv("reticulate", required = TRUE)
}
